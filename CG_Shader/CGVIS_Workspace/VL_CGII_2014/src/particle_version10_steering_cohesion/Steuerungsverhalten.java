...
public class Steuerungsverhalten {
   ...
   public Vektor2D randomForce() {
      return new Vektor2D(zuf.nextFloat() * 10 - 5,
            zuf.nextFloat() * 10 - 5);
   }

   public Vektor2D followMousePosition(Vektor2D currentPosition) {
      Vektor2D mousePosition = new Vektor2D(Mouse.getX(), Display
            .getDisplayMode().getHeight() - Mouse.getY());
   ...
   public Vektor2D separation(Flummi me, float dist) {
      ...
      for (int i = 0; i < me.objektManager.getFlummiSize(); i++) {
         ...
         if (bObj instanceof Flummi) {
            ...
            if (LineareAlgebra.euklDistanz(me.position,
                  bObjF.position) < dist) {
               Vektor2D help = LineareAlgebra.sub(me.position,
                     bObjF.position);
               float length = help.length();
               help.normalize();
               help.div(length);
